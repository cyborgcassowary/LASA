...
<form method="POST" action="Assign_Add_popup.asp" name="form1" webbot-action="--WEBBOT-SELF--">

<div align="center">
<br>
<b><font size="3">Create New Assignment
	</font></b>
	<br>
...
%>
<%do until rstemp.eof %>
<%If Project=rstemp(0) then sel="selected" else sel=""%>
...
conn.open "DSN=PDM;"%>
<%RS = Conn.Execute("Insert Into tbl_assign (Assignee,PID,A_Date,Title,Assignment,Duedate,Status,Priority,C_Date) Values ('" & Assignee & "','" & Project & "','" & A_Date & "','" & Assignment & "','" & Details & "','" & DueDate & "','" & Status & "','" & Priority & "','" & C_Date & "')")%>
<%
set RS=nothing
conn.close
Set conn=nothing
...
<%Details=Request.form("details")%>
<%duedate=Request.form("duedate")%>
<form method="POST" action="Assign_Add_popup.asp" name="Update" target="ibody">
