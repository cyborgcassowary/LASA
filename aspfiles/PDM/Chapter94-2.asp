...
set muni=munitemp.execute("Select Municipality from tbl_Municipality order by Municipality ASC;")
%>
<%do until muni.eof %>
	<tr>
		<td colspan="2" style="border-left-width: 1px; border-right-style: solid; border-right-width: 1px; border-top-style: solid; border-top-width: 1px; border-bottom-style: solid; border-bottom-width: 1px"><%=muni(0)%>&nbsp;</td>
...
set rstemp=conntemp.execute("Select Account,Permit,Development,P_Number,P_Street,DS_Pump,Taps from tbl_Permit where Municipality ='" & muni(0) & "' and Right(I_date,4) = Int(Year(Now))-1 and C_Type='" & CType & "' order by DS_Pump ASC;")
%>
<%do until rstemp.eof %>
<%X=X+1%>
