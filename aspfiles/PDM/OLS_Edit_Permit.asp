...
<%GISID=Request.QueryString("GISID")%>
<%I_Date=Request.QueryString("I_Date")%>
...
set rstemp=conntemp.execute("Select SDATE,Permit,Status,Plant,EDU,BuildDate from tbl_OLS where Permit='" & Permit & "';")
%>
<%do until rstemp.eof %>
<%SDATE=rstemp(0)%>
<%If Permit="" then Permit=rstemp(1)%>
<%Status=rstemp(2)%>
<%Plant=rstemp(3)%>
<%EDU=rstemp(4)%>
<%BDate=rstemp(5)%>
...
<%If I_Date <> "1/1/1900" then SDate=I_Date%>

<form method="POST" name="OLS" action="OLS_Edit_Permit.asp" webbot-action="--WEBBOT-SELF--">
<div align="center">
...
<%set conn=server.createobject("adodb.connection")
conn.open "DSN=PDM;"%>
<%RS = Conn.Execute("Update tbl_OLS Set GISID='" & GISID & "', SDATE='" & SDATE & "', Permit='" & Permit & "', Status='" & Status & "', Plant='" & Plant & "', EDU='" & EDU & "', BuildDate='" & BDate & "' where Permit='" & Permit & "';")%>
<%
set RS=nothing
conn.close
Set conn=nothing
