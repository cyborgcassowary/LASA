...
<body>
<div align="center">
<b><font size="3">Completed Assignments</font></b>
	<table border="0" cellpadding="0" cellspacing="0" width="100%" id="table1" style="border: 2px outset #0000FF">
		<tr>
			<td style="text-align: center; border-left-width:1px; border-right-width:1px; border-top-width:1px; border-bottom-style:solid; border-bottom-width:1px" colspan="2"><b>Assigned To</b></td>
...
set rstemp=conntemp.execute("Select ID,Assignee,PID,LastUpdate,DueDate,Status,Assignment,C_Date from tbl_assign where status = 'Complete' order by DueDate ASC;")
%>
<%do until rstemp.eof %>
<%X=X+1%>
<%If X/2=Int(X/2) then bgcolor="#C0C0C0" else bgcolor=""%>
<%if CDate(rstemp(7)) > CDate(rstemp(4)) then fntcolor="red" else fntcolor=""%>
